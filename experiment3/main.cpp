...
int main() {
    ...
    shmid = shmget((key_t)1234, sizeof(struct shared_use_ds),
    ...
    main_attach_addr = (struct shared_use_ds *)shmat(
        shmid, 0, 0);  //启动主进程main.cpp对共享内存的访问
    ...
    //下面创建信号灯
    semid = semget((key_t)5678, 2, IPC_CREAT | 0666);  //使用2个信号量
    // 0号：init=0,缓冲区已有数据数
    // 1号：init=N,空闲位置数
    arg.val = 0;
    ...
    //进程创建
    if ((readbuf = fork()) == 0) {
        printf("readbuf process created!\n");
        execl("./readbuf", "get", NULL);
        exit(0);
    } else if ((writebuf = fork()) == 0) {
        printf("writebuf process created!\n");
        execl("./writebuf", "put", NULL);
        exit(0);
    } else {  // father
        wait(NULL);
        wait(NULL);
